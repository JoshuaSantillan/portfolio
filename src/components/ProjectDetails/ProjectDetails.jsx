...
export const projects = [
  ...
  },
  {
    ...
    media: [
      ...
      { type: 'image', src: process.env.REACT_APP_PORTFOLIO_PROJECT_RESEARCH3_IMG_5, caption: 'Collecting this data lead to the development of a Carbon Aware Scheduler API that would return a list of the best optimal regions to run jobs in; these regions are the ones with lowest carbon intensity. One solution we used to utilize this API was Green Grader by grading jobs at in cleanest region rather than a random AWS server. By manipulating the use of their run_autograder docker script we were able to intercept the submission payload in transit to the AWS server and ingest it into our database at UCSD' },
      { type: 'image', src: process.env.REACT_APP_PORTFOLIO_PROJECT_RESEARCH3_IMG_6, caption: 'Our database works almost as the heart of our system as it is responsible for storing submissions, registering workers to run jobs, and storing the updated results after being graded. Utilizing daemons that are continously running in the cloud we can listen and passively collect new student submissions so as long as our code is injected into the run_autograder script. When a submission is recieved from ingestion it is sent over to the dispatcher component which will pair with a worker and get sent over to the scheduler. The scheduler will then have to make one of three choices. 1. Run the job at our UCSD server using docker. 2. Run a docker container in a K8s pod that is currently running in a region with low carbon intensity 3. Take advantage of a new innovative and sustainable "junkyard" phone cluster being built in cohort. This cluster repurposes smartphones by reinstalling their binaries to act as a distributed system and run jobs via docker and communicate via MQTT/RabbitMQ.' }
    ],
  ...
  },
  {
    ...
    media: [
      ...
      { type: 'video', src: process.env.REACT_APP_PORTFOLIO_PROJECTGL4_VIDEO_0, description: 'Unlike the Bezier Curve, the B Splines allow knot vectors which help determine the influence of all the control points and represent surfaces the Bezier Curve cannot such as Circles and Ellipses; they are are also affine invariance (preserve properties when scaled/translated/rotated).', controls: false },
